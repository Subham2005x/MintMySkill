...
const CourseCard = ({ course, showEnrollButton = true, showProgress = false }) => {
  ...
  if (!course) {
    return (
      <div className="card">
        <div className="text-white">Course data not available</div>
      </div>
    );
  }
  ...
  return (
    <div className="card hover:shadow-lg transition-shadow duration-300">
      ...
      <div className="relative mb-4">
        ...
        />
        <div className="absolute top-3 right-3 bg-gradient-to-r from-purple-600 to-purple-700 text-white px-2 py-1 rounded-full text-sm font-medium shadow-lg">
          +{tokenReward} tokens
        </div>
        {status && (
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center text-sm text-slate-400 mb-3">
          <span className="mr-4">{author || 'Unknown Author'}</span>
          <span className="mr-4">{duration || 'Duration not specified'}</span>
          <span>{Array.isArray(lessons) ? lessons.length : lessons || 0} lessons</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            {showEnrollButton ? (
              ...
              <span className="text-sm text-slate-400">{students || 0} students</span>
            )}
          </div>
